...
static int rmi_f34v7_read_flash_status(struct f34_data *f34)
{
	...
	ret = rmi_read_block(f34->fn->rmi_dev,
			...
			sizeof(status));
	if (ret < 0) {
		dev_err(&f34->fn->dev, "%s: Failed to read flash status\n",
			__func__);
		return ret;
	}
	...
	if (f34->v7.flash_status != 0x00) {
		dev_err(&f34->fn->dev, "%s: Flash status = %d, command = 0x%02x\n",
...
static int rmi_f34v7_read_queries_bl_version(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: Bootloader V%d.%d\n", __func__,
...
static int rmi_f34v7_read_queries(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: f34->v7.block_size = %d, [%d,%d]\n",
		 __func__, f34->v7.block_size, f34->bootloader_id[0],
		 f34->bootloader_id[1]);
	...
	if (f34->v7.flash_properties.has_config_id) {
		...
		snprintf(f34->configuration_id, sizeof(f34->configuration_id),
			 "%02x%02x%02x%02x",
			 f34_ctrl[0], f34_ctrl[1], f34_ctrl[2], f34_ctrl[3]);

		dev_info(&f34->fn->dev, "Configuration ID: %s\n",
...
static int rmi_f34v7_erase_configuration(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: Erase config command written\n", __func__);

	ret = rmi_f34v7_wait_for_idle(f34, ENABLE_WAIT_MS);
	if (ret < 0)
		return ret;

	rmi_dbg(RMI_DEBUG_FN, &f34->fn->dev, "%s: Idle status detected\n",
		__func__);
...
static int rmi_f34v7_erase_guest_code(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: Erase command written\n", __func__);
...
static int rmi_f34v7_write_flash_configuration(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: Erase flash configuration command written\n",
		 __func__);
...
static void rmi_f34v7_parse_image_header_10(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: f34->v7.img.checksum=%d\n",
	...
	for (ii = 0; ii < num_of_containers; ii++) {
		...
		dev_info(&f34->fn->dev, "%s: container_id=%d, length=%d\n",
...
static int rmi_f34v7_parse_image_info(struct f34_data *f34)
{
	...
	dev_info(&f34->fn->dev, "%s: header->major_header_version = %d\n",
...
int rmi_f34v7_do_reflash(struct f34_data *f34, const struct firmware *fw)
{
	...
	if (ret < 0)
		goto fail;
	dev_info(&f34->fn->dev, "%s: Firmware programmed\n", __func__);

	f34->v7.config_area = v7_UI_CONFIG_AREA;
	ret = rmi_f34v7_write_ui_configuration(f34);
	if (ret < 0)
		goto fail;
	dev_info(&f34->fn->dev, "%s: Configuration programmed\n", __func__);
	...
	if (f34->v7.flash_properties.has_disp_config &&
		...
		if (ret < 0)
			goto fail;
		dev_info(&f34->fn->dev, "%s: Display configuration programmed\n", __func__);
	}
	...
	if (f34->v7.new_partition_table) {
		if (f34->v7.has_guest_code && f34->v7.img.contains_guest_code) {
			...
			if (ret < 0)
				goto fail;
			dev_info(&f34->fn->dev, "%s: Guest code programmed\n", __func__);
		}
	}
...
int rmi_f34v7_start_reflash(struct f34_data *f34, const struct firmware *fw)
{
	...
	dev_info(&f34->fn->dev, "%s: start reflash process\n", __func__);
