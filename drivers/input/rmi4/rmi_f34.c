...
int rmi_f34_update_firmware(struct f34_data *f34, const struct firmware *fw)
{
	...
	dev_info(&f34->fn->dev, "Starting firmware update\n");
					     ...
					     char *buf)
{
	...
	if (fn) {
		...
		return scnprintf(buf, PAGE_SIZE, "%c%c\n",
				 f34->bootloader_id[0], f34->bootloader_id[1]);
	}
...
static DEVICE_ATTR(package_id, S_IRUGO,
                  rmi_driver_package_id_show, NULL);
